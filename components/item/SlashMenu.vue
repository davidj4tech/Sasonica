<template>
  ...
  <div v-if="commands.length" class="w-full mb-1.5 rounded-sm border border-border bg-primary overflow-hidden">
    <div v-for="command in commands" :key="command.name" class="flex items-baseline px-2 py-1.5 border-b border-border last:border-b-0" @click="command.unavailable || $emit('select', command)">
