...
// 5. ⋮ → Auto rename: POST {session, auto:true}; the server's name lands in the header and the list
await page.goto(BASE + '/t/' + shelved)
await page.getByRole('button', { name: 'Thread menu' }).click()
await page.getByRole('menuitem', { name: 'Auto rename' }).click()
await page.waitForSelector('text=Renamed “Named: Layout sketches”.')
ok((await page.locator('.title-button').innerText()) === 'Named: Layout sketches', 'header shows the name the server chose')
const ap = posts.filter((p) => p.p === '/rename').map((p) => JSON.parse(p.body)).find((b) => b.auto)
ok(ap && ap.session === shelved && ap.auto === true && !ap.title, 'POST /rename {session, auto: true}, no title')
await page.goto(BASE + '/threads')
await page.waitForSelector('.thread-row')
ok((await rowOf(shelved).locator('.title').innerText()) === 'Named: Layout sketches', 'list row carries the auto name')
await rowOf(working).dispatchEvent('contextmenu')
await page.getByRole('menuitem', { name: 'Auto rename' }).click()
await page.waitForTimeout(600)
ok((await rowOf(working).locator('.title').innerText()) === 'Named: Working, renamed', 'Auto rename from the list menu')
await page.screenshot({ path: SHOTS + '/rename-04-auto.png' })
