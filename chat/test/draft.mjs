...
// 7. The + over the send button: a new chat that takes what is typed.
await open(A)
await box().fill('this belongs elsewhere')
await page.locator('.new-chat-mini').click()
await page.waitForURL((u) => u.pathname === '/new')
await box().waitFor()
ok((await box().inputValue()) === 'this belongs elsewhere', '+: the words are in the new chat')
ok(new URL(page.url()).search === '', '+: a thread in no known place opens the picker unpicked')
await page.waitForTimeout(300)
ok((await server(A)) === '', '+: the thread keeps no copy')
await open(A)
ok((await box().inputValue()) === '', '+: the thread box is empty')
await box().fill('and this too')
await page.locator('.new-chat-mini').click()
await page.waitForURL((u) => u.pathname === '/new')
await box().waitFor()
ok((await box().inputValue()) === 'this belongs elsewhere\n\nand this too', '+: added to a new chat draft, not over it')
await page.screenshot({ path: SHOTS + '/draft-new-chat-carry.png' })
// In a known place, the new chat starts there.
await open(sid('Mock: long conversation'))
await page.locator('.new-chat-mini').click()
await page.waitForURL((u) => u.pathname === '/new')
ok(new URL(page.url()).searchParams.get('cwd') === '/home/you/projects/agent-media', `+: the thread's place is picked (${new URL(page.url()).search})`)
await page.waitForSelector('.bar h1')
ok((await page.locator('.bar h1').innerText()) === 'New chat in agent-media', '+: the header names the place')
