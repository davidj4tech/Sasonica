...
ok(JSON.stringify(await box.locator('button').allInnerTexts()) === JSON.stringify(['Interrupt', 'Auto speak', 'Normal', 'Quiet']), 'the four levels')
ok(JSON.stringify(await pressed()) === '["Normal"]', 'Normal pressed, as the server says')
...
await box.getByRole('button', { name: 'Normal' }).click()
await page.waitForFunction(() => document.querySelector('[data-testid=speech-default] button[aria-pressed=true]')?.textContent === 'Normal')
