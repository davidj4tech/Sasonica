...
ok(JSON.stringify(items) === JSON.stringify(['Share into…', 'Rename…', 'Auto rename', 'Move to project…', 'Speech…', 'Exit session', 'Exit & archive']), `live thread menu: ${items.join(' | ')}`)
...
ok(JSON.stringify(tItems) === JSON.stringify(['Insert a thread…', 'Rename…', 'Auto rename', 'Move to project…', 'Speech…', 'Exit session', 'Exit & archive']), `thread ⋮ menu: ${tItems.join(' | ')}`)
await page.getByRole('menuitem', { name: 'Exit session' }).click()
