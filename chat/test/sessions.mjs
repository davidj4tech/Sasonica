...
ok(JSON.stringify(items) === JSON.stringify(['Share into…', 'Rename…', 'Auto rename', 'Move to project…', 'Speech priority…', 'Exit session', 'Exit & archive']), `live thread menu: ${items.join(' | ')}`)
...
ok(JSON.stringify(tItems) === JSON.stringify(['Insert a thread…', 'Rename…', 'Auto rename', 'Move to project…', 'Speech priority…', 'Exit session', 'Exit & archive']), `thread ⋮ menu: ${tItems.join(' | ')}`)
await page.getByRole('menuitem', { name: 'Exit session' }).click()
...
await openSpeech()
ok((await page.locator('.speech-sheet [role=menuitemradio]', { hasText: 'When open' }).innerText()).includes('· default'), 'the default level is tagged')
ok((await page.locator('.speech-sheet .speech-default-line').innerText()).startsWith('Default for all threads: When open'), 'the sheet names the default')
...
ok((await useDefault.count()) === 1 && (await useDefault.innerText()).startsWith('Use the default (When open)'), '"Use the default (When open)" while it has its own')
