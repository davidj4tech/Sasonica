...
await page.waitForSelector('.action-sheet')
ok(JSON.stringify(await menuItems()) === JSON.stringify(['Rename…', 'Auto rename', 'Archive']), `shelved thread menu: no Exit (${(await menuItems()).join(' | ')})`)
...
await page.getByRole('button', { name: 'Thread menu' }).click()
ok(JSON.stringify(await page.locator('.menu [role=menuitem]').allInnerTexts()) === JSON.stringify(['Rename…', 'Auto rename', 'Unarchive']), 'its menu offers Unarchive')
