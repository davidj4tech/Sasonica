...
ok(JSON.stringify(items) === JSON.stringify(['Share into…', 'Rename…', 'Auto rename', 'Move to project…', 'Speech…', 'Exit session', 'Archive', 'Exit & archive']), `live thread menu: ${items.join(' | ')}`)
...
ok(JSON.stringify(tItems) === JSON.stringify(['Insert a thread…', 'Rename…', 'Auto rename', 'Move to project…', 'Speech…', 'Exit session', 'Archive', 'Exit & archive']), `thread ⋮ menu: ${tItems.join(' | ')}`)
await page.getByRole('menuitem', { name: 'Exit session' }).click()
...
await page.waitForSelector('.action-sheet')
ok(JSON.stringify(await menuItems()) === JSON.stringify(['Share into…', 'Rename…', 'Auto rename', 'Move to project…', 'Speech…', 'Archive']), `shelved thread menu: no Exit (${(await menuItems()).join(' | ')})`)
...
await page.getByRole('button', { name: 'Thread menu' }).click()
ok(JSON.stringify(await page.locator('.menu [role=menuitem]').allInnerTexts()) === JSON.stringify(['Insert a thread…', 'Rename…', 'Auto rename', 'Move to project…', 'Speech…', 'Unarchive']), 'its menu offers Unarchive')
...
// 9. Speech… (§6.4 /session/priority): four levels in a sheet from ⋮, a badge unless normal
