...
ok(JSON.stringify(tItems) === JSON.stringify(['Share into…', 'Rename…', 'Auto rename', 'Move to project…', 'Speech…', 'Exit session', 'Archive', 'Exit & archive']), `thread ⋮ menu: ${tItems.join(' | ')}`)
await page.getByRole('menuitem', { name: 'Exit session' }).click()
...
await page.getByRole('button', { name: 'Thread menu' }).click()
ok(JSON.stringify(await page.locator('.menu [role=menuitem]').allInnerTexts()) === JSON.stringify(['Share into…', 'Rename…', 'Auto rename', 'Move to project…', 'Speech…', 'Unarchive']), 'its menu offers Unarchive')
