...
ok(JSON.stringify(items) === JSON.stringify(['Share into…', 'Rename…', 'Auto rename', 'Move to project…', 'Always speak', 'Exit session', 'Archive', 'Exit & archive']), `live thread menu: ${items.join(' | ')}`)
...
ok(JSON.stringify(tItems) === JSON.stringify(['Share into…', 'Rename…', 'Auto rename', 'Move to project…', 'Always speak', 'Exit session', 'Archive', 'Exit & archive']), `thread ⋮ menu: ${tItems.join(' | ')}`)
await page.getByRole('menuitem', { name: 'Exit session' }).click()
...
await page.waitForSelector('.action-sheet')
ok(JSON.stringify(await menuItems()) === JSON.stringify(['Share into…', 'Rename…', 'Auto rename', 'Move to project…', 'Always speak', 'Archive']), `shelved thread menu: no Exit (${(await menuItems()).join(' | ')})`)
...
await page.getByRole('button', { name: 'Thread menu' }).click()
ok(JSON.stringify(await page.locator('.menu [role=menuitem]').allInnerTexts()) === JSON.stringify(['Share into…', 'Rename…', 'Auto rename', 'Move to project…', 'Always speak', 'Unarchive']), 'its menu offers Unarchive')
...
// 9. Always speak (§6.4 /session/priority): a toggle in the ⋮ menu, a badge while on
await page.goto(BASE + '/t/' + shelved)
await page.waitForSelector('.bar')
await page.getByRole('button', { name: 'Thread menu' }).click()
await page.getByRole('menuitem', { name: 'Always speak' }).click()
await page.waitForSelector('.badge.always-speak')
ok((await page.locator('.badge.always-speak').innerText()) === 'Always speaks', 'Always speak: the header says so')
const pp = posts.find((p) => p.p === '/session/priority')
ok(pp && pp.body.session === shelved && pp.body.priority === true, 'POST /session/priority {session, priority: true}')
ok((await rowFlags(shelved)).priority === true, 'the mock has it always speaking')
await page.screenshot({ path: SHOTS + '/sessions-06-always-speak.png' })
await page.getByRole('button', { name: 'Thread menu' }).click()
await page.getByRole('menuitem', { name: 'Stop always speaking' }).click()
await page.waitForTimeout(400)
ok((await page.locator('.badge.always-speak').count()) === 0 && (await rowFlags(shelved)).priority === false, 'Stop always speaking: badge gone, mock agrees')
