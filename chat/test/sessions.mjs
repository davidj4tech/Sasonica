...
ok(JSON.stringify(items) === JSON.stringify(['Rename…', 'Auto rename', 'Move to project…', 'Always speak', 'Exit session', 'Archive', 'Exit & archive']), `live thread menu: ${items.join(' | ')}`)
...
ok(JSON.stringify(tItems) === JSON.stringify(['Rename…', 'Auto rename', 'Move to project…', 'Always speak', 'Exit session', 'Archive', 'Exit & archive']), `thread ⋮ menu: ${tItems.join(' | ')}`)
await page.getByRole('menuitem', { name: 'Exit session' }).click()
...
await page.waitForSelector('.action-sheet')
ok(JSON.stringify(await menuItems()) === JSON.stringify(['Rename…', 'Auto rename', 'Move to project…', 'Always speak', 'Archive']), `shelved thread menu: no Exit (${(await menuItems()).join(' | ')})`)
...
await page.getByRole('button', { name: 'Thread menu' }).click()
ok(JSON.stringify(await page.locator('.menu [role=menuitem]').allInnerTexts()) === JSON.stringify(['Rename…', 'Auto rename', 'Move to project…', 'Always speak', 'Unarchive']), 'its menu offers Unarchive')
