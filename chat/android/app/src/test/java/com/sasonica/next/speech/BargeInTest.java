...
public class BargeInTest {
    ...
    // AudioManager's focus values, which the companion's FocusPolicy named.
    // Next never calls onFocus (the mic source settles it, and nothing here
    // holds focus on another player's behalf), but the evidence path is still
    // BargeIn's, and these are the cases that shaped it.
    private static final int GAIN = 1;
    private static final int LOSS = -1;
    private static final int LOSS_TRANSIENT = -2;
    private static final int LOSS_TRANSIENT_CAN_DUCK = -3;
    ...
    private static int aVoiceSessionDoesNot() {
        BargeIn b = new BargeIn();
        b.onMic(true, DICTATION, 1000);
        b.onFocus(LOSS_TRANSIENT, 2000);       // Cece starts talking
        ...
        b.onFocus(GAIN, 6000);                 // she finishes
    ...
    private static int aNotificationDingIsNotAConversation() {
        BargeIn b = new BargeIn();
        b.onMic(true, DICTATION, 1000);
        b.onFocus(LOSS_TRANSIENT_CAN_DUCK, 2000);
        b.onFocus(GAIN, 2000 + 400);           // a ding is short
    ...
    private static int aPauseForBreathDoesNotUndoIt() {
        ...
        b.onFocus(LOSS_TRANSIENT, 1000);
        b.onFocus(GAIN, 1000 + 700);           // one utterance
        int f = is(true, b.holding(2000), "700ms alone is under the floor");
        b.onFocus(LOSS_TRANSIENT, 3000);
        b.onFocus(GAIN, 3000 + 700);           // and another
    ...
    private static int closingTheMicClearsTheLatch() {
        BargeIn b = new BargeIn();
        b.onMic(true, DICTATION, 0);
        b.onFocus(LOSS_TRANSIENT, 100);
        b.onFocus(GAIN, 100 + 5000);
