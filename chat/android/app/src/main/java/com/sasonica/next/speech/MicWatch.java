...
// From agent-media's companion app by way of the Sasonica fork
// (com.audiobookshelf.app.speech), unchanged but for its package. Apache-2.0
// there and here; no Audiobookshelf-derived code.
...
final class MicWatch {
    ...
    void start() {
        if (audio == null) {
            android.util.Log.i("SasonicaMic", "mic: no AudioManager — not watching");
            return;
        }
        try {
            ...
            android.util.Log.i("SasonicaMic", "mic: watching (callback + poll)");
            poll();
        } catch (Throwable e) {
            // A probe is never worth the process. Same rule as the cards.
            android.util.Log.i("SasonicaMic", "mic: could not watch, carrying on: " + e);
            callback = null;
        }
    }
    ...
    void poll() {
        ...
        try {
            ...
            android.util.Log.i("SasonicaMic", "mic: poll failed: " + e);
        }
    }
    ...
    private void apply(List<AudioRecordingConfiguration> configs, String via) {
        ...
        android.util.Log.i("SasonicaMic", "mic: " + line);
